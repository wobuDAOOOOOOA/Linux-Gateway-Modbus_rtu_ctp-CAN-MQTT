...
{
           LOG_DEBUG("进入modbus_RTU创建连接函数");
                   LOG_INFO("modbus_RTU创建连接...");
...
int8_t retry=0;
int8_t rc;
LOG_DEBUG("进入modbus_RTU重连函数");

while(retry < RTU_MAXretry)
{
    if(*RTU_ctx == NULL)
    {
        LOG_ERROR("连接句柄为空，尝试第%d次重连\n",retry+1);
...
if(rc != -1)
{
  LOG_INFO("连接成功，返回%d个寄存器数据\n",rc);
  return rc;
}
    LOG_ERROR("连接失败,尝试释放并重新连接\n");
