...
int modbus_rtu_device_read(rtu_device_t *dev, int addr, int nb, uint16_t *dest)
{
    ...
    while(retry < RTU_MAXretry)
    {
        ...
        if (dev->ctx == NULL)
        {
            LOG_ERROR("RTU:句柄为空，第%d次重连,设备：%d", retry+1,dev->slave_id);
        ...
        // 2. 句柄有效，执行寄存器读取
        modbus_set_slave(dev->ctx, dev->slave_id);
        printf("xxxxxxxxx:%d",dev->slave_id);
                       // pthread_mutex_lock(&mgr.read_mutex);

        rc = modbus_read_registers(dev->ctx, addr, nb, dest);
                     //    pthread_mutex_unlock(&mgr.read_mutex);
        ...
        // 3. 读取失败，销毁连接，准备重试
        LOG_ERROR("RTU:读取失败 err=%s，销毁连接准备重试，设备号：%d", modbus_strerror(errno),dev->slave_id);
        // modbus_close(dev->ctx);
        // modbus_free(dev->ctx);

        modbus_flush(dev->ctx);   // 清空残留数据
        sleep(1);                 // 等待总线稳定
        dev->ctx = NULL;
        retry++;
