...
int mqtt_publish_data(float temperature, float humidity) {
    ...
    ret = mosquitto_publish(g_mosq, NULL, MQTT_PUB_TOPIC, strlen(payload), payload, 1, false);
...
int MQTT_publish(float temperature, float humidity) {
    ...
    // 设置用户名和密码（密钥认证的关键）
    ret = mosquitto_username_pw_set(g_mosq, MQTT_USERNAME, MQTT_PASSWORD);
    if (ret != MOSQ_ERR_SUCCESS) {
    ...
    // 连接到华为云 MQTT Broker
    ret = mosquitto_connect(g_mosq, MQTT_HOST, MQTT_PORT, 60);
    if (ret != MOSQ_ERR_SUCCESS) {
